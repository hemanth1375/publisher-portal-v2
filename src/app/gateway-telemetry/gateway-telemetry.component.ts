...
export class GatewayTelemetryComponent {
  ...
  gatewayTelemetryId : any;
  gatewayTelemetryData: any;


  constructor(private fb: FormBuilder, private getwayService:GatewayService, private toastService: ToastService,private route: ActivatedRoute) {
    this.TelemertyFormGroup = this.fb.group({
      isLoggingActive: [false],
      logginngLevel: ['ERROR'],
      loggingPrefix: ['[KRAKEND]'],
      enableLogstash: [false],
      logMsgFormat: ['default'],
      logStdOut: [true],
      logSysLog: [false],
      logSysLogFacility: ['local3'],
      isGelfActive: [false],
      GELFAdrress: [""],
      enableTCP: [false],
  ...
  ngOnInit() {
    ...
    this.route.parent?.paramMap.subscribe(params => {
      this.gatewayTelemetryId = params.get('id');
      console.log('Parent ID:', this.gatewayTelemetryId);
    });

    this.getwayService.getGtwyServiceSettings(this.gatewayTelemetryId).subscribe({
      next:(res)=>{
        console.log(res);
        this.gatewayTelemetryData = res;

        // this.tagsArray = this.gatewayTelemetryData?.extra_config?.["telemetry/opencensus"]?.exporters?.datadog?.tags || [];
        this.headersToPassArray = this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"]?.headers_to_pass || [];

        this.TelemertyFormGroup.patchValue({
          isLoggingActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/logging"],
          isGelfActive : !!this.gatewayTelemetryData?.extra_config?.["telemetry/gelf"],
          isOpenTelActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"],
          newRelicNativeSDKActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"],
          logginngLevel: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.level,
          loggingPrefix: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.prefix,
          logMsgFormat: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.format,
          logStdOut: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.stdout,
          logSysLog: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.syslog,
          logSysLogFacility: this.gatewayTelemetryData?.extra_config?.["telemetry/logging"]?.syslog_facility,
          enableLogstash : this.gatewayTelemetryData?.extra_config?.["telemetry/logstash"]?.enabled,
          GELFAdrress : this.gatewayTelemetryData?.extra_config?.["telemetry/gelf"]?.address,
          enableTCP : this.gatewayTelemetryData?.extra_config?.["telemetry/gelf"]?.enable_tcp,
          OTsampleRate: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.trace_sample_rate,
          OTreportingPeriod: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.metric_reporting_period,
          isMetricsAPiActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"],
          metricsDisableEndpoint: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.endpoint_disabled,
          metricsDisableProxy: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.proxy_disabled,
          metricsDisableRouter: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.router_disabled,
          metricsDisableBackend: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.backend_disabled,
          metricsListenAddress: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.listen_address,
          metricCollecTime: this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"]?.collection_time,
          newRelicLicense: this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"]?.license,
          newRelicSDKDebug: this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"]?.debug,
          otlp: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.exporters?.otlp,
          prometheus: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.exporters?.prometheus,
          prometheusActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/opencensus"]?.exporters?.prometheus,
          openCensusActive: !!this.gatewayTelemetryData?.extra_config?.["telemetry/opencensus"],
          openTelId: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.id,
          openTelExporterId: this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"]?.exporters?.id,
  ...
  submitTelemetry() {
    ...
    const telementryBody = {
      "name": this.TelemertyFormGroup.get('name')?.value,

      ...((this.TelemertyFormGroup.value?.isLoggingActive ) && {
        "telemetry/logging":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/logging"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/logging"].id}),
          "level": this.TelemertyFormGroup.value?.logginngLevel,
          "prefix":this.TelemertyFormGroup.value?.loggingPrefix,
          "syslog":this.TelemertyFormGroup.value?.logSysLog,
          "stdout":this.TelemertyFormGroup.value?.logStdOut,
          "format":this.TelemertyFormGroup.value?.logMsgFormat,
          "syslog_facility":this.TelemertyFormGroup.value?.logSysLogFacility
          }}),
      ...((this.TelemertyFormGroup.value?.isLoggingActive ) && {
        "telemetry/logstash":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/logstash"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/logstash"].id}),    
        "enabled" : this.TelemertyFormGroup.value?.enableLogstash
          }}),
      ...((this.TelemertyFormGroup.value?.isLoggingActive && this.TelemertyFormGroup.value?.isGelfActive ) && {
        "telemetry/gelf":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/gelf"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/gelf"].id}),    
        "address" : this.TelemertyFormGroup.value?.GELFAdrress,
        "enable_tcp" : this.TelemertyFormGroup.value?.enableTCP
          }}),


      ...((this.TelemertyFormGroup.value?.isOpenTelActive) && {
        "telemetry/opentelemetry":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/opentelemetry"].id}),    
        "trace_sample_rate" : this.TelemertyFormGroup.value?.OTsampleRate,
        "metric_reporting_period" : this.TelemertyFormGroup.value?.OTreportingPeriod,
          "exporters":{
      ...
      ...((this.TelemertyFormGroup.value?.isMetricsAPiActive) && {
        "telemetry/metrics":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/metrics"].id}),    
        "collection_time" : this.TelemertyFormGroup.value?.metricCollecTime,
        "proxy_disabled" : this.TelemertyFormGroup.value?.metricsDisableProxy,
        "router_disabled": this.TelemertyFormGroup.value?.metricsDisableRouter,
        "backend_disabled":this.TelemertyFormGroup.value?.metricsDisableBackend,
        "endpoint_disabled":this.TelemertyFormGroup.value?.metricsDisableEndpoint,
        "listen_address":this.TelemertyFormGroup.value?.metricsListenAddress,
          }}),
      ...((this.TelemertyFormGroup.value?.newRelicNativeSDKActive) && {
        "telemetry/newrelic":{
          ...(!!this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"] && {"id":this.gatewayTelemetryData?.extra_config?.["telemetry/newrelic"].id}),    
        "license" : this.TelemertyFormGroup.value?.newRelicLicense,
        "debug" : this.TelemertyFormGroup.value?.newRelicSDKDebug,
        "headers_to_pass": this.TelemertyFormGroup.value?.headersToPassArrayValue
          }}),
    ...
    this.getwayService.addTelemetry(this.gatewayTelemetryId, telementryBody).subscribe({
      next:(res : any)=>{
        console.log(res)
  ...
  createPrometheusConfigGroup(): FormGroup {
    return this.fb.group({
      promethusName: ["prometheus_exporter"],
      promethusPort: [9090]
    })
  }
